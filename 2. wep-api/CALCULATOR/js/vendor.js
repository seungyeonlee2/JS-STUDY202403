...
const opprateNumber = ({
  ...
  //연산 부호에 따라서 다른 연산 해야함 - 두 수와 부호 필요
  let result; // 연산 결과
  switch (mark) {
    case ADD:
      result = first + second;
...
const calculate = (type) => {
  ...
  // 2. 결과에 누적 (currentResult)
  currentResult = opprateNumber({ prevResult, type, enteredNumber }); //연산 결과 리턴 (opprateNumber())
