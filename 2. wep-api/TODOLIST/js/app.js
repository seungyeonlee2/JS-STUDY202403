...
function renderNewTodoElement({ id, text }) {
  ...
  // $newTodoLi.textContent = text; //택스트컨텐트하고 이너html하면 택스트컨텐트 지워짐
  // console.log($newTodoLi); //li 태그에 텍스트 들어가는지 확인
  //2-2. 클래스 추가하기
  ...
  $newTodoLi.innerHTML = ` 
                  <label class='checkbox'>
                    <input type='checkbox'>
                    <span class='text'>${text /*받아온텍스트들어가는곳*/}</span>
                  </label>
                  <div class='modify'>
...
function insertTodoData() {
  ...
  console.log($todoText); // 콘솔에 찍히는지 확인 - 얘는 'add' 클릭하면 찍힘
  const inputText = $todoText.value;
  // console.log(inputText); // 입력값 가져오는지 확인
...
function removeTodoData(dataId) {
  ...
  console.log(todos); // 배열에서 삭제됐는지 확인
}
...
// <button>에 타입 지정 안하면 form 안에 버튼은 type=submit이 자동 걸려있음
// 폼 안에 서브밋은 엔터 처도 작동

// 수정되면 배열 text, done 상태도 변경

// 되돌아가는 아이콘 - 수정모드 진입
// 두번째 클릭 - 수정완료
// 체크박스 이벤트 - click(x), change(o)

// 삭제 처리 진행
// 클릭한 버튼이 포함된 li를 탐색
// const $targetLi = document.querySelector('.todo-list-item'); -이렇게 하면 어떤 버튼을 눌러도 1번 버튼으로 선택됨

/* 이걸로 li 탐색 하기 */
// const $targetLi = e.target.parentElement.parentElement; 밑에 코드로 사용 (같음)
// const $targetLi = e.target.closest(".todo-list-item");

// console.log($targetLi); 검사

// 화면에서 제거
// document.querySelector('.todo-list').removeChild($targetLi); -밑에코드랑 같음
// $targetLi.remove();
